...
# Script version info.
#
gScriptVersion=15.7beta3
# ####################################################################################################
# ####################################################################################################
#
# Note that this version of ssdtPRGen is NOT an official release from Pike R. Alpha.
# Modified by Dolphin @ FireWolf Pl.
# Detailed modification and description can be found on 
# http://www.firewolf.science/2015/04/unofficial-ssdtprgen-sh-v15-7-beta3-with-broadwell-cpus-support/
#
# ####################################################################################################
# ####################################################################################################
...
function _initBroadwellSetup()
{
  ...
  # NEW DATA INJECTED BY Dolphin @ FireWolf Pl.
  # Note:
  # Based on C-States data of Haswell MacBook Pro models,
  # assume that MBP12,1 still follow the previous patterns:
  # gACST_CPU0 -> C1, C3, C6, C7, C8, C9 and C10
  # gACST_CPU1 -> C1, C2, C3, C6 and C7

  # gSystemType = 1 is for iMac and Mac Mini models
  # gSystemType = 3 is for Mac Pro models
  # gSystemType no need for MacBook Pro and MacBook Air models

  # Board-ID starts here
    ...
    # Now MacBook8,x are now supported by v15.7 beta3
...
function main()
{
  ...
  printf "             v15.6 Copyright (c) 2013-$(date "+%Y") by Pike R. Alpha\n"
  printf "\n"
  printf "             v$gScriptVersion modified by Dolphin @ FireWolf Pl.\n"
  printf "             Note that v$gScriptVersion is NOT an official release from Pike R. Alpha\n"
  echo   '-----------------------------------------------------------'
